...
const ModuleName = "raw-events"

// DAOInstance holds everything needed to process raw events for one DAO.
type DAOInstance struct {
	Config            DAOConfig
	DAO               *types.DAO
	Chain             *types.Blockchain
	ContractAddresses map[common.Address]*types.Address
	DeployedAt        uint64 // earliest deployment block among this DAO's contracts
}

// Module captures raw EVM event logs for DAOs without a specific governance module.
type Module struct {
	logger log.Logger
	db     Database

	// addressToDAO maps a contract address to its DAOInstance.
...
func Builder(
	...
	if !ok {
		return nil, fmt.Errorf("database must implement the raw-events Database interface")
	}
	...
	for _, daoCfg := range cfg.DAOs {
		...
		logger.Info().
			Str("dao", daoCfg.Symbol).
			Int("contracts", len(inst.ContractAddresses)).
			Msg("registered raw-events DAO")
	}

	if len(mod.daoInstances) == 0 {
		logger.Warn().Str("chain_id", chainID).Msg("no raw-events DAOs configured for this chain")
	}
...
func (m *Module) initDAO(ctx context.Context, daoCfg DAOConfig, chainID string) (*DAOInstance, error) {
	...
	contractAddresses := make(map[common.Address]*types.Address, len(daoCfg.Contracts))
	...
	for i := range daoCfg.Contracts {
		...
		contractAddresses[ethAddr] = addr
	}
	...
	for ethAddr := range contractAddresses {
