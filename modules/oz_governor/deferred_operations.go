...
func (m *Module) handleFetchProposalStatus(
	...
	var payload FetchOZGovernorProposalStatus
	if err := op.DecodePayload(&payload); err != nil {
		return err
	}

	proposal, err := m.db.GetProposalByProposalID(
		ctx, inst.DAO.ID, inst.Chain.ID, inst.Governor.Address.ID, payload.ProposalID,
	)
	if err != nil {
		return err
	}
	...
	proposalStatus, err := inst.Governor.Contract.GetProposalStatus(
		ctx, inst.Governor.Instance, payload.ProposalID, op.Height,
	)
	if err != nil {
